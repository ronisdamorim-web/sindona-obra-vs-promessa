...
export default function StatusObra({ data }: StatusObraProps) {
    ...
    return (
        <section className="min-h-screen md:h-screen w-full snap-start bg-zinc-950 flex items-center justify-center relative overflow-hidden text-stone-200 py-12 md:py-0">
            {/* Background Texture */}
            ...
            <div className="max-w-6xl w-full px-4 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start md:items-center relative z-10">
                ...
                <div className="space-y-8 md:space-y-12">
                    ...
                    >
                        <h2 className="font-serif text-3xl md:text-6xl text-white mb-2">Status Atual</h2>
                        <p className="text-stone-500 font-mono text-xs md:text-sm tracking-widest uppercase">Evolução Física da Obra</p>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-8">
                        <motion.div
                            ...
                            className="p-5 md:p-6 border border-white/10 bg-white/5 backdrop-blur-sm rounded-lg"
                        >
                            <div className="flex items-center gap-2 mb-3 md:mb-4 text-indigo-400">
                                <Ruler className="w-4 h-4 md:w-5 md:h-5" />
                                <span className="text-xs uppercase tracking-wider font-bold">Vitra Engenharia</span>
                            </div>
                            <div className="text-4xl md:text-5xl font-light text-white mb-2">{data.vitra.total}%</div>
                        ...
                        <motion.div
                            ...
                            className="p-5 md:p-6 border border-white/10 bg-white/5 backdrop-blur-sm rounded-lg"
                        >
                            <div className="flex items-center gap-2 mb-3 md:mb-4 text-emerald-400">
                                <Activity className="w-4 h-4 md:w-5 md:h-5" />
                                <span className="text-xs uppercase tracking-wider font-bold">Caixa Econômica</span>
                            </div>
                            <div className="text-4xl md:text-5xl font-light text-white mb-2">{data.caixa.total}%</div>
                    ...
                    >
                        ...
                        <p className="leading-relaxed">{data.nota}</p>
                    </motion.div>
                </div>

                {/* Direita: Detalhamento Vitra */}
                <div className="space-y-4 md:space-y-6">
                    <h3 className="text-stone-400 font-mono text-xs uppercase tracking-[0.2em] mb-4 md:mb-6 border-b border-white/10 pb-2">
                        ...
                        {etapas.map((etapa, idx) => (
                            ...
                            >
                                <div className="flex justify-between text-sm mb-2 text-stone-300">
                                    <span className="text-xs md:text-sm">{etapa.label}</span>
                                    <span className="font-mono text-stone-500 text-xs md:text-sm">{etapa.valor}%</span>
                                </div>
